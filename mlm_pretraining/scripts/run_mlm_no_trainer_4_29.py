...
RoBERTa...) on a text file or a dataset without using HuggingFace Trainer.

Here is the full list of checkpoints on the hub that can be fine-tuned by this
script:
    https://huggingface.co/models?filter=fill-mask
"""
# You can also adapt this script on your own mlm task. Pointers for this are
# left as comments.
...
# Will error if the minimal version of Transformers
# is not installed. Remove at your own risks.
...
def parse_args():
    ...
    parser.add_argument("--adam_eps", type=float, default=1e-08, help="Epsilon parameter of Adam optimizer.")
    ...
    parser.add_argument(
        "--fp16",
        action="store_true",
        help="Whether to enable mixed precision with FP16."
    )
...
def main():
    ...
    # # resources to maintain them. The information sent is the one passed as
    # # arguments along with your Python/PyTorch versions.
    ...
    if args.dataset_name is not None:
        ...
        if "validation" not in raw_datasets.keys():
            raw_datasets["validation"] = load_dataset(
                ...
                split=f"corpus[:{args.validation_split_percentage}%]",
            )
            ...
            # when processing corpus carolina, remove the corpus split
            # and use only train and validation to save compute.            <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            if 'corpus' in raw_datasets.keys():
                raw_datasets.pop('corpus')
    else:
        data_files = {}
        if args.train_file is not None:
    ...
    if args.line_by_line:
        ...
        # Otherwise, we tokenize every text, then concatenate
        # them together before splitting them in smaller parts. 
        # We use `return_special_tokens_mask=True` because
        # DataCollatorForLanguageModeling (see below) is more
    ...
    # Note -> the training dataloader needs to be prepared before we grab his
